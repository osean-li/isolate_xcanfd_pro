...
> - 关键电源/地脚位：
>   - **5 / 9 / 28 / 33 / 45 = VDD_SOC**（内核/系统电源）
>   - **6 / 34 = VIO_B00**（IO Bank 电源）
>   - **30 = VREFL**（接地参考，正常应对 GND 响）
>
> **HPM5321 电源脚分类（共 13 个电源脚，分属不同电源域）：**
>
> | 电源域 | 引脚 | 数量 | 说明 |
> |--------|------|------|------|
> | **VDD_SOC** | 5 / 9 / 28 / 33 / 45 | 5 | 内核/系统电源，5 个脚 |
> | **VIO_B00** | 6 / 34 | 2 | IO Bank 电源，2 个脚 |
> | **VANA** | —（模拟电源脚） | — | 模拟电源域 |
> | **VREFL** | 30 | 1 | 接地参考（接地参考，正常应对 GND 响） |
> | **VSS / EPAD** | 49（底部散热焊盘） | 1 | 接地，散热焊盘 |
>
> ⚠️ **不同电源域是独立网络，不直接连通**。排查"晶振不起振 / 烧写无应答"时，务必逐个电源域测电压：曾出现 **第 6 脚 VIO_B00 = 3.3V 正常、但第 5 脚 VDD_SOC = 0V（虚焊）** 的情况——VDD_SOC 没通电会导致 MCU 核心不工作、晶振不起振（晶振脚 0V）、ISP 烧写失败（`Read ack start status failed`）。
