...
### Q：上电后晶振不起振（两脚都 0V）、烧写 `Read ack start status failed` 是怎么回事？

A：这是 **U1 某个电源脚虚焊** 的典型连锁反应，不是晶振本身坏。定位与处理见下方「实战踩坑」案例。
...
## 8. 实战踩坑：第 5 脚（VDD_SOC）虚焊导致晶振不起振 / 烧写失败

> 真实定位过程 + 补焊注意事项，避免"晶振不起振"时误判为晶振坏或 U1 整体报废。

### 8.1 现象（一步步排查记录）

1. 板子上电后，测 **X1 两个晶振脚（XTAL0 / XTAL1）对 GND，均 = 0V**（正常起振应为 1.4~1.7V）。
2. 怀疑晶振坏 → 断电蜂鸣档测晶振脚对 GND → **不响（无短路）**，排除晶振短路。
3. 怀疑供电 → 测 `LDO1 VOUT` = **3.30V 正常**，排除系统供电问题。
4. 逐个电源域测 U1 电源脚：
   - 第 6 脚 `VIO_B00` = **3.30V**（正常）
   - 第 5 脚 `VDD_SOC` = **0V**（异常！）
5. 断电蜂鸣档测 **第 5 脚 ↔ LDO1 VOUT** → **不响（不通）**，其他电源脚互联正常。
6. **结论：第 5 脚（VDD_SOC）虚焊，内核电源没通。**

### 8.2 因果链（为什么晶振 0V、烧写失败）

```
第 5 脚 VDD_SOC 虚焊（内核电源 0V）
    → MCU 核心 / 内部反相器不工作
    → 晶振不起振（晶振脚 0V）
    → MCU 无系统时钟
    → ISP 无法响应（烧写报 Read ack start status failed）
```

> 关键认知：**无源晶振自己不产生信号，必须靠 U1 内部反相器驱动**。U1 内核没通电 → 反相器不工作 → 晶振脚就是 0V。所以"晶振不起振"不一定是晶振坏，**先查 U1 各电源域是否都通电**。

### 8.3 补焊第 5 脚的注意事项（重要，别桥锡）

第 5 脚紧贴 EPAD（GND）和第 6 脚（VIO_B00），补焊时**极易桥锡**，务必注意：

1. **断电**操作；
2. 烙铁 **320°C**，烙铁头**蘸一点助焊膏/焊油**（帮助锡流进 QFN 底部缝隙）；
3. 烙铁头**轻轻压在 5 脚与焊盘的接缝处**，保持 **2~3 秒**，让锡重新熔化润湿；**不要加一堆新锡**——5 脚旁边就是 EPAD（GND），锡多了会桥到地，从"虚焊"变"短路"；
4. 若需要补锡，用**极细锡丝**，尖端蘸一点点即可；
5. **顺便补焊其他 VDD_SOC 脚（9/28/33/45）**，防止它们同样虚焊（QFN 外围电源脚最容易虚焊，肉眼难辨）；
6. 温度别太高、时间别太长（320°C / 单脚 2~3 秒），防止烫坏芯片或把相邻脚带短路。

### 8.4 补焊后验证（按顺序全做）

| # | 操作 | 期望 |
|---|---|---|
| 1 | 断电，蜂鸣档：第 5 脚 ↔ LDO1 VOUT | **响**（通了） |
| 2 | 断电，蜂鸣档：第 5 脚 ↔ GND | **不响**（没桥到地） |
| 3 | 上电，量第 5 脚对 GND | **3.30V**（原 0V） |
| 4 | 上电，量两个晶振脚对 GND | **1.4~1.7V**（起振） |
| 5 | 烧写 | 按住 SW1 上电 → 出 COM 口 → 烧录成功 |

> 若补焊后第 5 脚仍 0V，说明 QFN 底部焊盘虚焊严重，需热风枪 310°C 整颗加热 30~40 秒重新回流，或拆下 U1 按本文档重新焊。
